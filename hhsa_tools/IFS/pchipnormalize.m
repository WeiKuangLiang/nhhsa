...
%
function [ndata,a]=pchipnormalize(data, alpha)
if nargin<2
    alpha=1;
end
% 0. set the default value about the  end-process methodtype
...
if alpha>1
    udata= spmmhh_resamplev(data,alpha);
    undata=zeros(size(udata));
    ua=zeros(size(udata));
    ...
    %5.Do end-process for bouble sides         
            ...
            umx=[umx(1);umx;umx(nExtrema)]; 

    %6.cubic Hermite spline interpolation for the envelope-AM 
            %find cubic Hermite spline envelope as AM                   
            ua(:,c)=pchip(utx,umx,ute);
            %***linear interpolation backup in case spline error. Added by KBB.
            %Experimental
            if(any(ua(:, c) < udata(:,c)))
                ua2 = interp1(utx, umx, ute);

                mask_sp = ones(length(ua), 1);

                for(i=1:length(utx)-1)
                    if(any(ua(utx(i):utx(i+1),c) < udata(utx(i):utx(i+1),c)))
                        mask_sp(utx(i):utx(i+1)) = 0;
                    end
                end

        %        mask_sp = (a(:,c) >= data(:, c)); 

                ua(:, c) = (ua(:,c) .* mask_sp) + (ua2 .* ~mask_sp);
            end      
            %end KBB

    %7.normalize the IMF,AM been devided out          
            %----- %  Normalize the data by splined envelope
            undata(:,c)=udata(:,c)./ua(:,c);


        else
            %----- Leave data unchanged
            ua(:,c)=ones(unpt,1);
            undata(:,c)=0*ones(unpt,1);
            %disp('Watch! there is a trend in the input Matrix')
        end
    %4. add check for trend--loop B end    
    end
    ndata=spmm_downsample(undata,alpha);
    a=spmm_downsample(ua,alpha);
else
    te=(1:npt)';  %a series of total number
